...
def translate_files(lang: str, files):
    ...
    for filepath in files:
        Path(str(filepath).replace(str(to_translate_dir), str(translated_dir))).parent.mkdir(exist_ok=True)
        if filepath.is_dir():
            ...
            with open(str(filepath).replace(str(to_translate_dir), str(translated_dir)), 'w') as file:
                file.write(str(html))
